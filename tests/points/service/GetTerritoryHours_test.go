...
type GetTerritoryHoursTestCase struct {
	Name           string
	UserId         int
...
var GetTerritoryHoursTestCases = []GetTerritoryHoursTestCase{
	{
		// GetTerritoryHoursCommand returns a database error. The error will return.
		Name:   "DatabaseError",
		UserId: 1,
		SetupMock: func() *dbpointsmock.DatabaseMock {
	...
	{
		// GetTerritoryHoursCommand succeeds and returns 0.
	...
	{
		// GetTerritoryHoursCommand succeeds and returns a positive value.
...
func TestSrvPoints_GetTerritoryHours(test *testing.T) {
	for _, testCase := range GetTerritoryHoursTestCases {
		test.Run(testCase.Name, func(test *testing.T) {
			...
			// Act
			points, err := sut.GetTerritoryHours(testCase.UserId)
